...
const CustomerReview = () => {
    ...
    return (
      <>
        { !reviews ? (<Loading/>):( <section className="py-10">
                ...
                {reviews?.map((review) => {
                    return (
                        <div key={review.id} className="carousel-item ml-3 mr-3 bg-white gap-2 border border-solid border-gray-300 rounded-xl p-4 transition-all duration-500 mx-auto">
                            ...
                            <p className="text-base text-gray-600 leading-6 pb-8">{review.message}</p>
                            <div className="flex items-center gap-5 border-t border-solid border-gray-200 pt-5">
                                <img className="rounded-full h-10 w-10" src={review.image} alt="avatar" />
                                <div className="block">
                                    <h5 className="text-gray-900 font-medium mb-1">{review.username}</h5>
